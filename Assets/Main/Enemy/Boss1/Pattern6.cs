...
class Boss1Pattern_TearsGeyser : BossPattern<Boss1>{
  ...
  public override void Execute(Boss1 caller){
    if (tearsCooldown.Try()){
      ...
      for (int i = 0; i < projectileCount; i++){
        ...
        rb.position = caller.rb.position;
  ...
  public Boss1Pattern_TearsGeyser(){
    var blueprint = AssetDatabase.LoadAssetAtPath(Constants.Prefabs.DefaultEnemyProjectile, typeof(GameObject)) as GameObject;
    var blueprint2 = AssetDatabase.LoadAssetAtPath(Constants.Prefabs.EnemyEffectorProjectile, typeof(GameObject)) as GameObject;
    tearsPool = new GameObjectPool(blueprint, 300, 1000) {
      Parent = new GameObject("Boss1: Storm Troopers Tear Ducts > Tears"),
    };
    geyserPool = new GameObjectPool(blueprint2, 20, 50) {
      Transform = ResetGeyserProjectile,
      Parent = new GameObject("Boss1: Storm Troopers Tear Ducts > Breakdowns"),
    };
